...
class ViewController: UIViewController, IMAAdsLoaderDelegate, IMAStreamManagerDelegate {
    ...
    controller.videoPlayer?.addPeriodicTimeObserver(
      ...
      using: { (time: CMTime) -> Void in
        if self.videoPlayer != nil, let item = self.videoPlayer!.currentItem {
          Task {
            let duration = await controller.getPlayerItemDuration(item)
  ...
  func streamManager(_ streamManager: IMAStreamManager, didReceive event: IMAAdEvent) {
    ...
    case IMAAdEventType.SHOW_AD_UI:
      print("Show ad UI.")
      let customUI = event.customUI
      let ad = event.ad
      if customUI != nil && ad != nil {
  ...
  func getPlayerItemDuration(_ item: AVPlayerItem) async -> CMTime {
    if item.status == .readyToPlay {
      return item.duration
    }
    do {
      return try await item.asset.load(.duration)
    } catch {
